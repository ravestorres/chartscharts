            ...
            <b-card-body>
              ...
              <b-table
                ...
                <template #cell(week)="row">
                  <span class="d-block">Week {{ items[row.index].week_number || row.item.week_number || row.index + 1 }}</span>
                  <span class="d-block sub smaller">{{ items[row.index].week_start || row.item.week_start }}</span>
                  <span class="d-block sub smaller">{{ items[row.index].week_end || row.item.week_end }}</span>
                </template>
                <template #cell(playcount)="row">
                  <span class="d-block">{{ items[row.index].playcount || row.item.playcount }}</span>
                </template>
                <template #cell(link)="row">
                  <b-button variant="dark" size="sm" :to="{ name: 'weekly.' + tp, params: { week: items[row.index].week || row.item.week } }">
                    {{ $t('word.view_week') }}
                  </b-button>
                </template>
              </b-table>
  ...
  methods: {
    ...
    loadCharts() {
      ...
      if (weeks.length > 0) {
        ...
        // Add week information to items if not already present
        if (this.items.length > 0 && weeks.length > 0) {
          this.items = this.items.map((item, index) => {
            const weekData = weeks[index] || {};
            return {
              ...item,
              week_number: weekData.weekNumber || index + 1,
              week_start: weekData.start ? moment(weekData.start).format('MMM D, YYYY') : '',
              week_end: weekData.end ? moment(weekData.end).format('MMM D, YYYY') : '',
              week: weekData.weekNumber || index + 1,
